...
static __u64 handle_extent (struct file_extent ext, unsigned int sectors_per_block, __u64 start_lba)
{
...
static long long int walk_fibmap (int fd, struct stat *st, unsigned int blksize, unsigned int sectors_per_block, __u64 start_lba)
{
	...
	return handle_extent(ext, sectors_per_block, start_lba);
}
...
static long long int walk_fiemap (int fd, unsigned int sectors_per_block, __u64 start_lba)
{
...
__u64 do_filemap (const char *file_name)
{
	int fd, err;
	struct stat st;
	__u64 start_lba = 0;
	...
	long long int lba;
	lba = walk_fiemap(fd, sectors_per_block, start_lba);
	if (lba < 0)
		lba= walk_fibmap(fd, &st, blksize, sectors_per_block, start_lba);
	close (fd);
	return lba;
}
